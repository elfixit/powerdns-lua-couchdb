...
function(doc, req) {
    ...
    var soa_rr = function (nameserver, hostmaster, serial, refresh, retry, expire, minimum){
        return ("@ IN SOA " + nameserver + ". " + hostmaster + ". ("+ serial+" "+ refresh + " "+ retry + " "+ expire +" "+ minimum +")\n\n");
    };
    ...
    zone += soa_rr(nameserver, hostmaster, serial, refresh, retry, expire, minimum);
    ...
    if (doc.rr && doc.rr.length > 0) {
        for (var i = 0; i < doc.rr.length; i++) {
            ...
            if (rr_type === "MX" && rr.priority){
                var rr_data = rr.priority + " " + rr.content;
            } else {
                var rr_data = rr.content;
                if (rr_type === "TXT"){
                    // Quote here
                    rr_data = "\""+rr_data+"\"";
                }
